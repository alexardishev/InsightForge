...
const ColumnRenameSuggestionsPage: React.FC = () => {
  ...
  const fetchSuggestions = useCallback(async () => {
    ...
    try {
      const data = await request<ColumnRenameSuggestionsResponse>(
        `http://localhost:8888/api/column-rename-suggestions?${params.toString()}`
      );
  ...
  const handleAccept = async (id: number) => {
    ...
    try {
      await request(`http://localhost:8888/api/column-rename-suggestions/${id}/accept`, 'POST');
  ...
  const handleReject = async (id: number) => {
    ...
    try {
      await request(`http://localhost:8888/api/column-rename-suggestions/${id}/reject`, 'POST');
